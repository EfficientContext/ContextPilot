...
class TreeNode:
    ...
    def __init__(self, id: Optional[int] = None, priority: int = 0):
        ...
        # Request tracking: set of request IDs that have extra (unique) tokens in this node.
        # When this set becomes empty after eviction, the request only has shared tokens remaining.
...
class RadixCache(BasePrefixCache):
    ...
    def insert(
        ...
        return self._insert_helper(self.root_node, key, value, priority, request_id)
    ...
    def cache_finished_req(self, req: Req, is_insert: bool = True):
        ...
        if is_insert:
            ...
            # Pass request ID to track which request owns these extra tokens
            new_prefix_len = self.insert(
                radix_key, values, priority=priority, request_id=req.rid
            )
    ...
    def evict(self, num_tokens: int):
        ...
        num_evicted = 0
        # Track requests whose extra tokens have been fully evicted
        fully_evicted_requests: set = set()
        ...
        while num_evicted < num_tokens and len(eviction_heap):
            ...
            # Collect request_ids from this node - these requests lose their extra tokens
            if x.request_ids:
                fully_evicted_requests.update(x.request_ids)
                # Clean up request -> node mapping
                for rid in x.request_ids:
                    if rid in self._request_to_node:
                        del self._request_to_node[rid]
        ...
        # Invoke callback with set of fully evicted request IDs
        if fully_evicted_requests and self.eviction_callback is not None:
            try:
    ...
    def set_eviction_callback(self, callback):
        ...
        The callback will be invoked with a set of request IDs whenever those
        requests' extra tokens are fully evicted from the cache (meaning only
        shared prefix tokens remain, if any).

        Args:
            callback: A callable that accepts a set of request IDs (strings),
                      or None to disable the callback.

        Example:
            def on_eviction(evicted_request_ids: set):
                for rid in evicted_request_ids:
                    my_index.remove_request(rid)
    ...
    def get_tracked_request_ids(self) -> set:
        """
        Get the set of request IDs that have extra tokens currently in cache.

        Returns:
            Set of request IDs (strings) that have unique tokens stored in the
            radix tree. Requests that only share prefix tokens with others are
    ...
    def is_request_in_cache(self, request_id: str) -> bool:
        ...
        Returns:
            True if the request has unique tokens stored in the cache,
            False otherwise (request was evicted or never cached, or only
            has shared prefix tokens remaining).
        """
        return request_id in self._request_to_node

    def get_request_node(self, request_id: str) -> Optional[TreeNode]:
        """
        ...
        Returns:
            The TreeNode containing this request's extra tokens, or None if
            the request is not in cache or has been evicted.
        """
    ...
    def _split_node(self, key: RadixKey, child: TreeNode, split_len: int):
        ...
        # Request IDs stay with child (which still has the extra/unique tokens).
        # new_node represents shared prefix and gets no request_ids.
        # No changes needed: child.request_ids remains intact, new_node.request_ids is empty.
    ...
    def _insert_helper(
        ...
        if len(key):
            # New tokens to add - create new leaf node
            ...
            # Track request_id on the new node (where extra tokens are stored)
            # Only requests with EXTRA tokens need tracking - full matches use
            # the same cached content and will be handled when that content is evicted
            if request_id is not None:
                new_node.request_ids.add(request_id)
                self._request_to_node[request_id] = new_node

        # If len(key) == 0, request fully matches existing cache.
        # No need to track separately - the request uses the same cached content
        # as the request that created this node. When that node is evicted,
        # the original request_id will be notified, which is sufficient for
        # the context index since they represent the same cached context.
...
if __name__ == "__main__":
    # Example demonstrating request tracking and eviction callbacks
    evicted_requests = []

    def on_eviction(request_ids: set):
        print(f"Eviction callback: requests evicted = {request_ids}")
        evicted_requests.extend(request_ids)

    tree = RadixCache.create_simulated()
    tree.set_eviction_callback(on_eviction)

    # Example token id sequences with request tracking
    tree.insert(RadixKey(token_ids=[1, 2, 3], extra_key=None), request_id="req-001")
    tree.insert(RadixKey(token_ids=[1, 2, 3, 4], extra_key=None), request_id="req-002")
    tree.insert(RadixKey(token_ids=[1, 2, 4, 5], extra_key=None), request_id="req-003")
    tree.insert(RadixKey(token_ids=[1, 2, 4, 5, 6, 7], extra_key=None), request_id="req-004")
    tree.insert(RadixKey(token_ids=[8, 9, 10, 11, 12], extra_key=None), request_id="req-005")

    print("Tree structure:")
    tree.pretty_print()

    print("\nTracked requests:", tree.get_tracked_request_ids())
    print("Is req-003 in cache?", tree.is_request_in_cache("req-003"))

    print("\nMatching prefix [1, 2, 3, 13, 14]:")
