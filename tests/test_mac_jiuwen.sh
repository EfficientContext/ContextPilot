...
# Inference runs through openjiuwen WorkflowAgent instead of a raw OpenAI client.
#
# Usage:
#   bash tests/test_mac_jiuwen.sh                        # full run, 100 queries
#   bash tests/test_mac_jiuwen.sh --num-queries 50       # fewer queries
#   bash tests/test_mac_jiuwen.sh --skip-data-prep       # skip steps 1-2 (data already built)
#   bash tests/test_mac_jiuwen.sh --model path/to/model.gguf
#   bash tests/test_mac_jiuwen.sh --llama-server /path/to/custom/llama-server
#
# Prerequisites:
...
MODEL="${MODEL:-models/Llama-3.2-1B-Instruct-Q4_K_M.gguf}"
...
OUTPUT_CP="${OUTPUT_CP:-results_jiuwen_contextpilot.jsonl}"
OUTPUT_BASELINE="${OUTPUT_BASELINE:-results_jiuwen_baseline.jsonl}"
...
python -c "import openjiuwen" 2>/dev/null || \
    die "openjiuwen not importable. Run: pip install openjiuwen"

python -c "import elasticsearch" 2>/dev/null || \
    die "elasticsearch package not found. Run: pip install elasticsearch"
...
contextpilot-llama-server -m "$MODEL" \
    ...
    > /tmp/llama_server_jiuwen.log 2>&1 &
LLAMA_PID=$!
PIDS+=("$LLAMA_PID")
log "llama-server PID=$LLAMA_PID  (log: /tmp/llama_server_jiuwen.log)"
...
python -m contextpilot.server.http_server \
    ...
    > /tmp/contextpilot_server_jiuwen.log 2>&1 &
CP_PID=$!
PIDS+=("$CP_PID")
log "ContextPilot HTTP server PID=$CP_PID  (log: /tmp/contextpilot_server_jiuwen.log)"

wait_for_http "http://localhost:${CP_PORT}/" "ContextPilot server" 30

# ── Step 6: Run ContextPilot benchmark (openjiuwen) ───────────────────────────
hr
...
# ── Step 7: Restart llama-server to clear KV cache for fair baseline ──────────
hr
...
# ── Step 8: Run baseline benchmark (openjiuwen) ───────────────────────────────
hr
...
echo ""
echo "  Service logs:"
echo "    llama-server      : /tmp/llama_server_jiuwen.log"
echo "    ContextPilot HTTP : /tmp/contextpilot_server_jiuwen.log"
echo ""
