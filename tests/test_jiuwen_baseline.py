"""
Baseline: docs in original retrieval order, no ContextPilot reordering.
Run this first (cold cache), then restart llama-server, then run test_jiuwen_reorder.py.
"""
import os
import time
import asyncio
...
async def main():
    ...
    # No reordering — original retrieval order
    context = "\n\n".join(retrieved_docs)
    print(f"[Baseline] Context: {context}")
    ...
    print(f"[Baseline] Time: {elapsed:.3f}s")
