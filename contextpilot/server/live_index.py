...
class ContextPilot(ContextIndex):
    ...
    def optimize(
        ...
        *,
        ...
        """Reorder documents and return ready-to-use OpenAI messages.

        A convenience wrapper around :meth:`reorder` that also handles
        prompt assembly so callers don't have to build the messages
        themselves.

        Example::

            cp_live = cp.ContextPilot(use_gpu=False)
            messages = cp_live.optimize(docs, query)
        ...
        Returns:
            A list of message dicts (``role`` / ``content``) ready for
            the OpenAI chat completions API.
        """
        from ..api import _build_system_prompt

        reordered, _indices = self.reorder(
            docs, conversation_id=conversation_id
        )
        reordered_docs = reordered[0]
    ...
    def optimize_batch(
        ...
        *,
        ...
        Returns:
            ...
            corresponds to ``all_queries[original_indices[i]]``.
        """
        from ..api import _build_system_prompt
        ...
        reordered_contexts, order = self.reorder(all_docs)
