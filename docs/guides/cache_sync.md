...
Block-level dedup operates on the message content before forwarding. It doesn't depend on cache state — it simply finds identical blocks across tool results and replaces duplicates with pointers. Works the same whether the backend is SGLang, OpenAI, or Anthropic.
